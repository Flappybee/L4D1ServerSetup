...
public OnPluginStart()
{
	...
	g_hReturnToLobby		= CreateConVar("vb_returntolobby",		"b",	"0 - Allow this type of vote, x - Only clients that match one or more of these flags can call this vote",						FCVAR_PLUGIN, true, 0.0, true, 0.0);
		...
		if (GetClientTeam(client) == 1 && !IsClientAdmin(client) && GetConVarBool(g_hSpectatorVote))
		{
			...
			for (new x = 1; x <= MaxClients; x++)
			{
				if (IsClientInGame(x) && !IsFakeClient(x))
				{
					if (client == x)
					{
						PrintToChat(x, "%s 作为旁观者不能发起投票!", PREFIX);
					}

					else if (IsClientAdmin(x))
					{
						PrintToChat(x, "%s %s (旁观者)试图发起投票!", PREFIX, g_sCallerName[client]);
					}

					else
					{
						PrintToChat(x, "%s 成功阻止了一位旁观者发起的投票!", PREFIX);
					}
				}
			}
			...
			if (GetConVarInt(g_hBlockCount) > 0)
			{
				...
				{
					PrintToChatAll("%s 踢出了 %s,因为其发起太多次不被允许的投票.", PREFIX, g_sCallerName[client]);
					KickClient(client, "你发起了太多次不被允许的投票");
				}
			}
		...
		if (StrEqual(g_sIssue[client], "kick", false))
		{
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起踢人投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了一个踢人投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止发起踢人投票!", PREFIX);
						}
					}
				}
			...
			if (GetConVarBool(g_hAdminImmunity))
			{
				...
				if (GetConVarBool(g_hRespectLevels) && callerid != INVALID_ADMIN_ID && targetid != INVALID_ADMIN_ID && !CanAdminTarget(callerid, targetid))
				{
					...
					PrintToChat(client, "%s 你不能踢出一个具有更高权限的玩家!", PREFIX);
					PrintToChat(target, "%s %s 想踢出你!", PREFIX, g_sCallerName[client]);
			...
			else if (GetConVarBool(g_hTankImmunity) && IsPlayerTank(target))
			{
				...
				PrintToChat(client, "%s 你不能踢出一个扮演坦克的玩家!", PREFIX);
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起返回大厅投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了返回大厅投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止了发起返回大厅投票!", PREFIX);
						}
					}
				}
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起改变全体语音投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了改变全体语音设置的投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止了发起改变全体语音设置的投票!", PREFIX);
						}
					}
				}
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起重新开始战役投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了重新开始战役的投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止了发起重新开始战役的投票!", PREFIX);
						}
					}
				}
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起开始新战役的投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了开始新战役的投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止了发起开始新战役的投票!", PREFIX);
						}
					}
				}
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起改变章节投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了改变章节投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止了发起改变章节投票!", PREFIX);
						}
					}
				}
			...
			if (StrContains(sFlags, "0") == -1 && !FlagMatch(client, sFlags))
			{
				...
				for (new x = 1; x <= MaxClients; x++)
				{
					if (IsClientInGame(x) && !IsFakeClient(x))
					{
						if (client == x)
						{
							PrintToChat(x, "%s 不允许发起改变游戏难度投票!", PREFIX);
						}

						else if (IsClientAdmin(x))
						{
							PrintToChat(x, "%s %s 发起了改变游戏难度投票!", PREFIX, g_sCallerName[client]);
						}

						else
						{
							PrintToChat(x, "%s 成功阻止了发起改变游戏难度投票!", PREFIX);
						}
					}
				}
