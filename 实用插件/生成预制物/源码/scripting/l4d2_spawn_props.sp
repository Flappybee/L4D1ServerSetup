...
public Action CmdDebugProp(int client, int args)
{
	...
	{
		CPrintToChat(client, "[提示] %T","You are not looking at a valid object", client);
	...
	CPrintToChat(client, "[提示] %T", "Object Model", client, Object, m_ModelName);

	static char name[256];
	GetEntPropString(Object, Prop_Data, "m_iName", name, sizeof(name));
	CPrintToChat(client, "[提示] %T", "Object Targetname", client, name);

	static float position[3];
	GetEntPropVector(Object, Prop_Send, "m_vecOrigin", position);
	CPrintToChat(client, "[提示] %T", "Object Position", client, position[0], position[1], position[2]);

	static float angle[3];
	GetEntPropVector(Object, Prop_Data, "m_angRotation", angle);
	CPrintToChat(client, "[提示] %T", "Object Angle", client, angle[0], angle[1], angle[2]);
...
public Action CmdSpawnProp(int client, int args)
{
	...
	if(args < 3)
	{
		CPrintToChat(client, "[提示] Usage: sm_spawnprop <model> [static | dynamic | physics] [cursor | origin]");
		return Plugin_Handled;
	}
	char arg1[256];
	char arg2[256];
	...
	if(!IsModelPrecached(model))
	{
		...
		{
			CPrintToChat(client, "[提示] There was a problem spawning the selected model [ERROR: Invalid Model]");
			return Plugin_Handled;
		}
	}
	...
	if(strcmp(arg2, "static", false) == 0)
	{
		...
		if(strcmp(arg3, "cursor") == 0)
		{
			...
			{
				CPrintToChat(client, "[提示] Vector out of world geometry. Spawning on current position instead");
			}
		}
		...
		{
			CPrintToChat(client, "[提示] Invalid spawn method specified. Use: [cursor | origin]");
			return Plugin_Handled;
		}
		VecAngles[0] = 0.0;
		VecAngles[2] = 0.0;
	...
	{
		CPrintToChat(client, "[提示] Invalid render mode. Use: [static | dynamic | physics]");
		return Plugin_Handled;
	}
	return Plugin_Handled;
}
...
public int MenuHandler_DA_Ask(Menu menu, MenuAction action, int param1, int param2)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			{
				DeleteAllProps();
				CPrintToChat(param1, "[提示] %T", "Successfully deleted all spawned objects", param1);
			}
			else
			{
				CPrintToChat(param1, "[提示] %T", "Canceled", param1);
			}
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(strcmp(menucmd, "sm_spdeleteall")== 0)
			{
				...
				CPrintToChat(param1, "[提示] %T", "delete all the spawned objects?", param1);
			}
	...
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(g_iCategory[param1] == 1)
			{
				...
				{
					CPrintToChat(param1, "[提示] Vector out of world geometry. Spawning on current position instead");
				}
				VecAngles[0] = 0.0;
				VecAngles[2] = 0.0;
				DispatchKeyValueVector(prop, "angles", VecAngles);
		...
		case MenuAction_Select:
		{
			...
			switch(g_iMoveCategory[param1])
			{
				case 1:
				{
					...
					if(Object <= MaxClients || !IsValidEntity(Object))
					{
						CPrintToChat(param1, "[提示] %T","You haven't locked anything yet", param1);
...
void DeleteLookingEntity(int client)
{
	...
	CPrintToChat(client, "[提示] %T", "Successfully removed an object", client, Object);
...
void DeleteLastProp(int client)
{
	...
	{
		CPrintToChat(client, "[提示] %T", "You haven't spawned anything yet", client);
		return;
	}
	...
	if(Object > MaxClients && IsValidEntity(Object))
	{
		...
		|| strcmp(class, "upgrade_laser_sight", false) == 0)
		{
			...
			CPrintToChat(client, "[提示] %T","Succesfully deleted the last spawned object",client);
			g_iLastObject[client] = -1;
			g_bSpawned[Object] = false;
			g_bUnsolid[Object] = false;
		...
		{
			CPrintToChat(client, "[提示] %T", "The last spawned object index is not an object anymore!", client, Object);
			g_iLastObject[client] = -1;
			g_bSpawned[Object] = false;
			g_bUnsolid[Object] = false;
		}
	...
	{
		CPrintToChat(client, "[提示] %T","The last object is not valid anymore",client);
	}
}
...
void SaveMapStripper(int client)
{
	...
	if(file == null)
	{
		CPrintToChat(client, "[提示] Failed to create or overwrite the map file");
		CPrintToChat(client, "{green}[提示] Failed to create or overwrite the map file");
		PrintHintText(client, "[提示] Failed to create or overwrite the map file");
		PrintToConsole(client, "[提示] Failed to create or overwrite the map file");
		PrintCenterText(client, "[提示] Failed to create or overwrite the map file");
		return;
	}
	
	CPrintToChat(client, "{green}[提示] %T", "Saving the content. Please Wait", client);
	...
	FlushFile(file);
	CloseHandle(file);
	CPrintToChat(client, "{lightgreen}[提示] %T (%s/maps/%s.cfg)", "Succesfully saved the map data", client, g_sCvar_stripper_cfg_path, map);
}
...
public Action CmdRotate(int client, int args)
{
	...
	if(args < 2)
	{
		CPrintToChat(client, "[提示] Usage: sm_prop_rotate <axys> <angles> [EX: !prop_rotate x 30]");
		return Plugin_Handled;
	}
	//int Object = g_iLastObject[client];
	...
	if(Object <= MaxClients || !IsValidEntity(Object))
	{
		CPrintToChat(client, "[提示] %T","You are not looking at a spawned object", client);
		return Plugin_Handled;
	}
	...
	{
		CPrintToChat(client, "[提示] Invalid Axys (x,y,z are allowed)");
	}
	TeleportEntity(Object, NULL_VECTOR, vecAngles, NULL_VECTOR);
...
public Action CmdRemoveAll(int client, int args)
{
	...
	CPrintToChat(client, "{green}[提示] %T","Are you sure(Delete All)?",client);
...
public Action CmdMove(int client, int args)
{
	...
	if(args < 2)
	{
		CPrintToChat(client, "[提示] Usage: sm_prop_move <axys> <distance> [EX: !prop_move x 30]");
		return Plugin_Handled;
	}
...
public Action CmdSetAngles(int client, int args)
{
	...
	if(args < 3)
	{
		CPrintToChat(client, "[提示] Usage: sm_prop_setang <X Y Z> [EX: !prop_setang 30 0 34]");
		return Plugin_Handled;
	}
...
public Action CmdSetPosition(int client, int args)
{
	...
	if(args < 3)
	{
		CPrintToChat(client, "[提示] Usage: sm_prop_setpos <X Y Z> [EX: !prop_setpos 505 -34 17");
		return Plugin_Handled;
	}
...
public Action CmdLock(int client, int args)
{
	...
	CPrintToChat(client, "[提示] %T", "Succesfully locked spawned object", client, Object);
...
public Action CmdClone(int client, int args)
{
	...
	if(Object <= MaxClients || !IsValidEntity(Object))
	{
		CPrintToChat(client, "[提示] %T", "You haven't locked anything yet", client);
		return Plugin_Handled;
	}
