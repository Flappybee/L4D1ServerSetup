...
Action CmdMachineGunSave(int client, int args)
{
	...
	if( !hFile.ImportFromFile(sPath) )
	{
		PrintToChat(client, "%s错误: 读取重机枪配置文件失败, 找不到文件(\x05%s\x01).", CHAT_TAG, sPath);
	}
	...
	{
		PrintToChat(client, "%s错误: 将地图添加到重机枪配置文件失败.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	if( iCount >= MAX_ALLOWED )
	{
		PrintToChat(client, "%s错误: 不可添加更多重机枪. 已添加: (\x05%d/%d\x01).", CHAT_TAG, iCount, MAX_ALLOWED);
		delete hFile;
		return Plugin_Handled;
	}
	...
	PrintToChat(client, "%s(\x05%d/%d\x01) - 保存坐标:[\x05%f %f %f\x01] 角度:[\x05%f %f %f\x01]", CHAT_TAG, iCount, MAX_ALLOWED, vPos[0], vPos[1], vPos[2], vAng[0], vAng[1], vAng[2]);
	return Plugin_Handled;
}
...
Action CmdMachineGunList(int client, int args)
{
	...
	if( client == 0 )
		PrintToChat(client, "[提示] 重机枪数量: %d.", count);
	else
...
Action CmdMachineGunDelete(int client, int args)
{
	...
	if( index == -1 )
	{
		PrintToChat(client, "%s附近没有重机枪可删除!", CHAT_TAG);
		return Plugin_Handled;
	}
	...
	if( !FileExists(sPath) )
	{
		PrintToChat(client, "%s警告: 找不到重机枪配置文件 (\x05%s\x01).", CHAT_TAG, CONFIG_SPAWNS);
		return Plugin_Handled;
	}
	...
	if( !hFile.ImportFromFile(sPath) )
	{
		PrintToChat(client, "%s警告: 无法加载重机枪配置文件 (\x05%s\x01).", CHAT_TAG, sPath);
		delete hFile;
		return Plugin_Handled;
	}
	...
	if( !hFile.JumpToKey(sMap) )
	{
		PrintToChat(client, "%s警告: 当前地图不在重机枪配置文件中.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	for( int i = 1; i <= iCount; i++ )
	{
		...
		if( !bMove )
		{
			...
			{
				PrintToChat(client, "%s警告: 在重机枪配置文件中无内容.", CHAT_TAG);
				delete hFile;
				return Plugin_Handled;
			}
		}
	...
	PrintToChat(client, "%s(\x05%d/%d\x01) - 重机枪从配置文件中移除.", CHAT_TAG, iCount, MAX_ALLOWED);
	return Plugin_Handled;
}
...
Action CmdMachineGunClear(int client, int args)
{
	...
	ResetPlugin();
	PrintToChat(client, "%s所有重机枪从此地图中移除.", CHAT_TAG);
	return Plugin_Handled;
}
...
Action CmdMachineGunWipe(int client, int args)
{
	...
	if( !FileExists(sPath) )
	{
		PrintToChat(client, "%s错误: 找不到重机枪配置文件 (\x05%s\x01).", CHAT_TAG, sPath);
		return Plugin_Handled;
	}
	...
	if( !hFile.ImportFromFile(sPath) )
	{
		PrintToChat(client, "%s错误: 重机枪配置文件加载失败 (\x05%s\x01).", CHAT_TAG, sPath);
		delete hFile;
		return Plugin_Handled;
	}
	...
	{
		PrintToChat(client, "%s错误: 当前地图不在重机枪配置文件中.", CHAT_TAG);
		delete hFile;
		return Plugin_Handled;
	}
	...
	ResetPlugin();
	PrintToChat(client, "%s(0/%d) - 所有重机枪从配置中删除, 通过命令 \x05sm_mgsave\x01添加新重机枪配置.", CHAT_TAG, MAX_ALLOWED);
	return Plugin_Handled;
}
