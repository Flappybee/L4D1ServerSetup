...
{
	CreateConVar("l4d_selfhelp_version", PLUGIN_VERSION, " ", 0|FCVAR_DONTRECORD);
	
	l4d_selfhelp_incap = CreateConVar("l4d_selfhelp_incap", "3", "self help for incap , 0:disable, 1:pill, 2:medkit, 3:both  ", 0);
	l4d_selfhelp_grab = CreateConVar("l4d_selfhelp_grab", "3", " self help for grab , 0:disable, 1:pill, 2:medkit, 3:both ", 0);
	l4d_selfhelp_pounce = CreateConVar("l4d_selfhelp_pounce", "3", " self help for pounce , 0:disable, 1:pill, 2:medkit, 3:both ", 0);
	l4d_selfhelp_ride = CreateConVar("l4d_selfhelp_ride", "3", " self help for ride , 0:disable, 1:pill, 2:medkit, 3:both ", 0);
	l4d_selfhelp_pummel = CreateConVar("l4d_selfhelp_pummel", "3", "self help for pummel , 0:disable, 1:pill, 2:medkit, 3:both  ", 0);
	l4d_selfhelp_edgegrab = CreateConVar("l4d_selfhelp_edgegrab", "3", "self help for edgegrab , 0:disable, 1:pill, 2:medkit, 3:both  ", 0);
	l4d_selfhelp_eachother = CreateConVar("l4d_selfhelp_eachother", "1", "incap help each other , 0: disable, 1 :enable  ", 0);
	l4d_selfhelp_pickup = CreateConVar("l4d_selfhelp_pickup", "1", "incap pick up , 0: disable, 1 :enable  ", 0);
	l4d_selfhelp_kill = CreateConVar("l4d_selfhelp_kill", "1", "kill attacker", 0);

	l4d_selfhelp_hintdelay = CreateConVar("l4d_selfhelp_hintdelay", "3.0", "hint delay", 0);
	l4d_selfhelp_delay = CreateConVar("l4d_selfhelp_delay", "1.0", "self help delay", 0);
	l4d_selfhelp_durtaion = CreateConVar("l4d_selfhelp_durtaion", "3.0", "self help duration", 0);

	l4d_selfhelp_versus = CreateConVar("l4d_selfhelp_versus", "1", "0: disable in versus, 1: enable in versus", 0);	
...
public Action:AdvertisePills(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x03长按 \x04ctrl键\x03 进行自救");
	}
...
SelfHelp(client)
{
	...
	if(slot!=-1)
	{
		...
		else if(slot==3)
		{
			...
			HelpState[client]=STATE_OK;
			PrintToChatAll("\x04%N\x03 使用医疗包进行了自救!", client); 
 ...
 HelpOther(client, helper)
{
	...
	PrintToChatAll("\x04%N\x03 帮助了\x04 %N \x03 恢复行动", helper, client);  
	//first(client);
 }
ReviveClientWithKid(client)
{
