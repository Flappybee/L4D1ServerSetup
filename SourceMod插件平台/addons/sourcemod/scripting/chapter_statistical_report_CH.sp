...
public Action Command_ResetStatistics(int client, int args) {
    ResetCampaignStats();
    PrintToChat(client, "\x04[数据统计] \x01数据统计已经成功 \x03重置\x01.");
...
public void PrintClientStats(int client) {
    if (g_bPrintMode) {
        PrintToChat(client, "\x04[数据统计] \x01个人统计数据已打印在 \x03控制台\x01.");
        PrintToConsole(client, "\x04[数据统计] \x03Chapter Statistical Report");
    } else {
        PrintToChat(client, "\x04[数据统计] \x03角色数据报告");
    }
    ...
    if (IS_SURVIVOR(client)) {
        ...
        if (g_bPrintMode) {
            ...
            PrintToChat(client, "\x04[!] \x03%s \x01- \x04普感 \x01(\x05%d\x01) \x01- \x04特感 \x01(\x05%d\x01) \x01- \x04爆头 \x01(\x05%d\x01) \x01- \x04爆头率 \x01(\x05%i%%\x01)", clientName, g_iChapterCIKills[client], g_iChapterSIKills[client], g_iChapterHSKills[client], roundedHSPct);
        }
    }
}
...
public void PrintTeamStats(int client) {
    if (g_bPrintMode) {
        PrintToChat(client, "\x04[数据统计] \x01团队统计数据已打印在 \x03控制台\x01.");
        PrintToConsole(client, "\x04[数据统计] \x03Chapter Statistical Report");
    } else {
        PrintToChat(client, "\x04[数据统计] \x03角色数据报告");
    }
        ...
        if (g_bOnlyHumanPlayer) {
            if (IS_HUMAN_SURVIVOR(i)) {
                ...
                if (g_bPrintMode) {
                    ...
                    PrintToChat(client, "\x04[!] \x03%s \x01- \x04普感 \x01(\x05%d\x01) \x01- \x04特感 \x01(\x05%d\x01) \x01- \x04爆头 \x01(\x05%d\x01) \x01- \x04爆头率 \x01(\x05%i%%\x01)", clientName, g_iChapterCIKills[i], g_iChapterSIKills[i], g_iChapterHSKills[i], roundedHSPct);
                }
            }
        } else {
            if (IS_SURVIVOR(i)) {
